...
- (void)configBaseView{
    ...
    flow.minimumLineSpacing         = UNITHEIGHT * 10;
    flow.minimumInteritemSpacing    = UNITHEIGHT * 10;
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == _dataArr.count) {
        return CGSizeMake(self.frame.size.width, UNITHEIGHT * 40);
    }
...
- (UIEdgeInsets)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout insetForSectionAtIndex:(NSInteger)section{
    return UIEdgeInsetsMake(UNITHEIGHT * 10, UNITHEIGHT * 20, UNITHEIGHT * 10, UNITHEIGHT * 20);
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == _dataArr.count) {
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, UNITHEIGHT * 40)];
        /* 判断最后一个item的内容 如果没有历史记录 内容就为暂无历史记录  否则为清除历史记录 */
        label.text = (_dataArr.count==0?(@""):(@""));
    ...
    CGSize size                         = [str boundingRectWithSize:CGSizeMake(self.frame.size.width, 1000) options:NSStringDrawingUsesFontLeading|NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingTruncatesLastVisibleLine attributes:dict context:nil].size;
    UILabel *label                      = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, size.width, UNITHEIGHT * 40)];
    label.text                          = str;
	...
	label.textColor = [UIColor colorWithHexString:@"#8f8e94"];
    [cell.contentView addSubview:label];
	label.textAlignment = NSTextAlignmentCenter;
