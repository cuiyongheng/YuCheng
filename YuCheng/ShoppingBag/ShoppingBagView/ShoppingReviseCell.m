...
@implementation ShoppingReviseCell
...
//	self.dingjinLabel = [[UILabel alloc] init];
//	_dingjinLabel.text = @"定金 ¥7000元";
//	_dingjinLabel.font = font(14);
//	[self.contentView addSubview:_dingjinLabel];
...
- (void)layoutSubviews {
	...
	[_picImageView mas_makeConstraints:^(MASConstraintMaker *make) {
		make.left.mas_equalTo(UNITHEIGHT * 80);
...
//	}];
	...
	[_moneyView mas_makeConstraints:^(MASConstraintMaker *make) {
		...
		make.height.mas_equalTo(UNITHEIGHT * 124);
	}];
	...
	[_chooseButton mas_makeConstraints:^(MASConstraintMaker *make) {
		make.centerX.mas_equalTo(self.contentView.mas_left).with.offset(UNITHEIGHT * 40);
		make.centerY.mas_equalTo(self.contentView);
		make.height.width.mas_equalTo(UNITHEIGHT * 25);
	}];

//	[_dingjinLabel mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (void)setModel:(ProductModel *)model {
	...
	_moneyLabel.text = [NSString stringWithFormat:@"%@", model.goods_price];
//	_dingjinLabel.text = [NSString stringWithFormat:@"订金 %@", model.deposit];
}
